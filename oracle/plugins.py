...
class Loader:
    ...
    def process_command(self, bot, input):
        ...
        bot.l_say('%s is not recognised as an $nick$ command.' \
    ...
    def try_command(self, bot, input, m):
        ...
        try:
            ...
            traceback.print_exc()
            bot.l_say('Sorry, something went wrong with that command.', input, 0)
            bot.l_say('The error has been logged.', input, 0)
            return
